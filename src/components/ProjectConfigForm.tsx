...
import {
  useLineBalancingStore,
  ProjectConfig,
} from "@/store/lineBalancingStore";
...
const projectConfigSchema = z.object({
  ...
  productionTimePerDay: z.coerce
    .number()
    .min(1, "Debe ser mayor a 0")
    .max(1440, "Máximo 1440 minutos (24 horas)"),
  requiredProductionPerDay: z.coerce
    .number()
    .min(1, "Debe ser mayor a 0")
    .max(100000, "Valor muy alto"),
});
...
export const ProjectConfigForm = ({ onComplete }: ProjectConfigFormProps) => {
  const setProjectConfig = useLineBalancingStore(
    (state) => state.setProjectConfig
  );

  const form = useForm<ProjectConfigFormValues>({
    resolver: zodResolver(projectConfigSchema),
    defaultValues: {
      projectName: "Línea de Ensamble Principal",
  ...
  return (
    <Card className="shadow-lg border-primary/20">
      <CardHeader className="bg-gradient-to-r from-primary/5 to-accent/5 border-b">
        ...
        <CardDescription>
          Ingrese los parámetros básicos de producción
        </CardDescription>
      </CardHeader>
